...
[Ct,Ht] = chain_tail(unimer_Ct,unimer_Ht,b);
Cd(:,:) = U(:,:,d)*Cc;
Hd(:,:) = U(:,:,d)*Hc;
Nd(:,:) = U(:,:,d)*Nc;
Od(:,:) = U(:,:,d)*Oc;
...
CC = reshape(Cd,[3,DP*nC*1]);
HH = reshape(Hd,[3,DP*nH*1]);
NN = reshape(Nd,[3,DP*nN*1]);
OO = reshape(Od,[3,DP*nO*1]);



CT = reshape(Ct,[3,nCt*1]);
HT = reshape(Ht,[3,nHt*1]);
ST = reshape(St,[3,nSt*1]);
